...
def find_download_link(contents):
    match = re.search(r"https://addons\.mozilla\.org/firefox/downloads/file/\d+/.+?\.xpi", contents)
    if match:
        #print(match.group(0))
        return match.group(0)
    else:
        print("Error: Unable to find download link for addon.")
        sys.exit(1)
...
def process(addon):
    ...
    addon_xpi_data = addon_xpi.read()
    install(addon_xpi_data, guid)


def install(addon_xpi, guid):
    profile_path = os.path.expanduser("~/.var/app/org.mozilla.firefox/.mozilla/firefox/**.default-release/")
    dest = glob.glob(profile_path)
    #print("Profile directory:", dest)  # Print the profile directory for inspection
